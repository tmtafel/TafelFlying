...
namespace TafelFlyingServices.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        protected string GetRoleNameFromRoleId(string id)
        {
        ...
        protected string GetRoleNameFromUserId(string userId)
        {
            return (from r in _db.Roles
                where r.Users.Any(a => a.UserId == userId)
                select r.Name).FirstOrDefault();
        }

        protected string GetRoleIdFromUserId(string userId)
        {
            return (from r in _db.Roles
                where r.Users.Any(a => a.UserId == userId)
        ...
        protected void AddOrRemoveFromRole(string userId, string roleId)
        {
            if (UserManager.IsInRole(userId, roleId)) return;
            foreach (IdentityRole role in _db.Roles)
            {
                if (role.Id != roleId)
                {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (roleName != "Admin") return RedirectToAction("Index", "Account");
        ...
        public ActionResult Edit(string id)
        {
            ...
            ApplicationUser userId = _db.Users.Find(id);
            ...
            var editUser = new EditViewModel
            {
                ...
                Id = userId.Id,
                ViewToPublic = userId.ViewToPublic,
                IsAPilot = userId.IsAPilot,
                RoleId = GetRoleIdFromUserId(userId.Id)
            };
        ...
        {
            if (!ModelState.IsValid) return View(idModel);
            ...
            if (result.Succeeded)
            {
                ...
                if (GetRoleNameFromRoleId(idModel.RoleId) != "Admin") return RedirectToAction("Index");
                List<Plane> planes = _db.Planes.ToList();
                List<string> claims =
